...
func TestConsensusFixedOracle(t *testing.T) {
	...
	cfg := config.Config{N: 16, F: 8, RoundDuration: time.Second * time.Duration(1)}
...
func TestSingleValueForHonestSet(t *testing.T) {
	...
	cfg := config.Config{N: 50, F: 25, RoundDuration: time.Second * time.Duration(1)}
	totalNodes := 50
	sim := service.NewSimulator()
	test.initialSets = make([]*Set, totalNodes)
	set1 := NewSetFromValues(value1)
...
func TestAllDifferentSet(t *testing.T) {
	...
	cfg := config.Config{N: 10, F: 5, RoundDuration: time.Second * time.Duration(1)}
	sim := service.NewSimulator()
	totalNodes := 10
	test.initialSets = make([]*Set, totalNodes)
...
func TestSndDelayedDishonest(t *testing.T) {
	...
	cfg := config.Config{N: 50, F: 25, RoundDuration: time.Second * time.Duration(2)}
	totalNodes := 50
	sim := service.NewSimulator()
	test.initialSets = make([]*Set, totalNodes)
